...
class RandomNameGenerator
  ...
  def dump_probability_tables(json_file_name = "media/probability_table.json")
    File.open(json_file_name, "w") do |f|
      f.puts '{'
      f.puts '  "pair_probability_table": '
      f.puts JSON.pretty_generate(@pair_probability_table.frequencies)
      f.puts ','
      f.puts '  "triplet_probability_table": '
      f.puts JSON.pretty_generate(@triplet_probability_table.frequencies)
      f.puts '}'
    end
  end
